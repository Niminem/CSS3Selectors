...
import std/[strutils, unicode, math, parseutils, sets]
...
func node(pair: NodeWithContext): Node =
    return pair.parent.childList[pair.index]
...
iterator siblings(pair: NodeWithContext,
                  startAtIndex = 0): Element =
    if pair.parent != nil:
        var idx = startAtIndex
        while idx < pair.parent.childList.len:
            let node = pair.parent.childList[idx]
            if node of Element:
                yield Element(node)
            idx.inc
...
func hasAttr(node: Node, attr: string): bool {.inline.} =
    if not (node of Element): return false
    let
        e = Element(node)
        attrs = e.getAttrs()
    return (e.attrs.len > 0) and attr in attrs
...
func satisfies(pair: NodeWithContext, demand: Demand): bool =
    ...
    of tkAttributeItem:
        return node.hasAttr(demand.attrName) and
            ...
            demand.attrValue in node.getAttr(demand.attrName).split(CssWhitespace)

    # Empty attrValue is allowed,
    # and will match any value starting with '-'
    of tkAttributePipe:
        return node.hasAttr(demand.attrName) and
            demand.attrValue == node.getAttr(demand.attrName).split("-")[0]

    of tkAttributeExact:
        return node.getAttr(demand.attrName) == demand.attrValue

    of tkAttributeStart:
        return demand.attrValue.len > 0 and
            node.getAttr(demand.attrName).startsWith(demand.attrValue)

    of tkAttributeEnd:
        return demand.attrValue.len > 0 and
            node.getAttr(demand.attrName).endsWith(demand.attrValue)

    of tkAttributeSubstring:
        return demand.attrValue.len > 0 and
            node.getAttr(demand.attrName) in demand.attrValue

    of tkElement:
        return $node.tagType() == demand.element
    ...
    of tkPseudoOnlyOfType:
        for sibling in pair.siblings:
            if sibling != node and sibling.tagType() == node.tagType():
                return false
        return true
    ...
    of tkPseudoFirstOfType:
        for sibling in pair.siblings:
            if sibling.tagType() == node.tagType():
                return sibling == node
    ...
    of tkPseudoNthOfType:
        ...
        for sibling in pair.siblings:
            if sibling == node:
                break
            elif sibling.tagType() == node.tagType():
...
func exec*(query: Query, node: Element, single: bool): seq[Element] =
    ...
    var stack = @[NodeWithContext(parent: node, index: 0, elementIndex: 0, searchStates: initialStates )]
    ...
    while stack.len > 0:
        ...
        if entry.parent != nil:
            ...
            while idx < entry.parent.childList.len and not (entry.parent.childList[idx] of Element):
                idx.inc
            if idx < entry.parent.childList.len:
        ...
        if entry.node.childList.len > 0:
            var idx = 0
            while idx < entry.node.childList.len and not (entry.node.childList[idx] of Element):
                idx.inc
            if idx < entry.node.childList.len:
                stack.add NodeWithContext(
                    parent: entry.node,
                    index: idx,
                    elementIndex: 0,
...
func querySelector*(root: seq[Node], queryString: string, # MADE BY ME (supporting HTML Fragment)
                    options: set[QueryOption] = DefaultQueryOptions): Element
                    {.raises: [ParseError].} =
    ## Get the first element matching `queryString`,
    ## or `nil` if no such element exists.
    ## Raises `ParseError` if parsing of `queryString` fails.
    let query = parseHtmlQuery(queryString, options)
    let lst = query.exec(root.makeElemRoot(), single = true)
    if lst.len > 0:
        lst[0]
    else:
        nil
...
func querySelectorAll*(root: seq[Node], queryString: string,
                       options: set[QueryOption] = DefaultQueryOptions):
                       seq[Element] {.raises: [ParseError].} =
    ## Get all elements matching `queryString`.
    ## Raises `ParseError` if parsing of `queryString` fails.
    let query = parseHtmlQuery(queryString, options)
    result = query.exec(root.makeElemRoot(), single = false)
